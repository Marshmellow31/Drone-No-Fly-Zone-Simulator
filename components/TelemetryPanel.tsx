...
const TelemetryItem = ({ label, value, unit, valueClassName }: { label: string; value: string | number; unit: string; valueClassName?: string }) => (
  <div className="flex justify-between items-baseline text-gray-300">
    <span className="text-sm font-medium text-gray-400">{label}</span>
    <span className={`font-mono text-lg font-semibold ${valueClassName || 'text-white'}`}>
    ...
    <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-4">
      ...
      <div className="space-y-2">
        ...
        <TelemetryItem 
          ...
          unit="%" 
          valueClassName={getBatteryColor(telemetry.battery)} 
        />
      </div>
    </div>
  );
